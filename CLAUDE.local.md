...
- **スクリプトは `pnpm run <名前>` で呼ぶ（`run` を省かない）。** `pnpm pack` は pnpm 組み込みの tarball 作成が走り、`scripts.pack` は実行されない。エラーにならず `white-box-0.1.0.tgz` ができるだけなので、配布物を作ったつもりで作れていない状態に気づけない。
...
- **契約の args スキーマは strict のまま置く（`z.strictObject` / `.strict()`）。** 緩い `z.object` に戻すと、綴り違いのキーを zod が黙って捨て、ハンドラは空の patch を実行する。画面は保存できた顔をして何も変わらない（型検査も lint もテストも通ったまま無反応になる）。
