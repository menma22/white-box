...
`shots-baseline/` の 11 枚（今日 / ボード / 履歴 / 設定 / Welcome / 開始 / HUD / 満了 / レビュー / 現在の仕事 / タスク詳細）は、Quri 形式への再編を始める前の見た目。**「見た目は変えていない」はずの変更**では、ここへ撮り直したものを重ねて差分を見る。時計の表示は撮るたびに変わるので、そこだけは差が出てよい。
