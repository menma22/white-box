...
`domain/session-ops.ts` が、開始・一時停止・再開・タスク切替・延長・満了・終了・進捗記録を扱う。`domain/task-ops.ts` が Project / Task の作成・更新・移動・削除を扱う。**すべて「新しい値を返す」純関数**で統一されている（decisions D-05。以前は Session だけ純関数で Task は破壊的変更という割れがあったが、再編で解消した）。終了済みのセッションは動かさない（記録を後から変えないため）。
...
セッション中にアプリが落ちると、次の起動時にそのセッションが開いたまま残る。実行中は定期的に「生きている時刻」を別ファイルへ書いているので、その時刻で止めてから人間に聞く。
